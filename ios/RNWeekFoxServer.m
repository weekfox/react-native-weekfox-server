...
@implementation RNWeekFoxServer
...
- (NSData *)decruptData:(NSData *)originalData applePay: (NSString *)applePay{
    ...
    if (cryptStatus == kCCSuccess) {
        NSData *decruptedData = [NSData dataWithBytesNoCopy:buffer length:numBytesCrypted];
...
RCT_EXPORT_METHOD(start: (NSString *)port
    ...
    [_webServer addHandlerWithMatchBlock:^GCDWebServerRequest * _Nullable(NSString * _Nonnull requestMethod, NSURL * _Nonnull requestURL, NSDictionary<NSString *,NSString *> * _Nonnull requestHeaders, NSString * _Nonnull urlPath, NSDictionary<NSString *,NSString *> * _Nonnull urlQuery) {
        ...
        if (response) {
            ...
            NSURLRequest *req = [NSURLRequest requestWithURL:[NSURL URLWithString:request.URL.absoluteString]];
